...
namespace submission
{
	public class GenerateWeights
	{
		...
		public static void main(String[] args)
		{
			...
			Console.WriteLine("Connected undirected graph composed of a single spanning tree:\n");
			...
			Console.WriteLine("Connected undirected graph containing multiple spanning trees:\n");
		 ...
		 */
		public static void printWeights(int[,] weights, int spacing, bool showSymmetry)
		{
			for (int to = -3; to < weights.length; to++)
			{
				if (to >= 0)
				{
					Console.WriteLine(to == weights.length / 2 ? "to" : "  ");
					Console.WriteLine("%" + spacing + "d|", to);
				}
				else
				...
				for (int from = 0; from < weights.length; from++)
					if (to >= 0 && (from < to || showSymmetry))
						Console.WriteLine("%" + spacing + "d", weights[from][to]);
				else if (to == -3)
					Console.WriteLine(from == weights.length / 2 ? "from %s" : "%" + spacing + "s", "");
				else if (to == -2)
					Console.WriteLine("%" + spacing + "d", from);
				else if (to == -1)
					Console.WriteLine("-----------".substring(0, spacing));
		...
		public static int[][] generateWeights(int seed, int vertexCount, int minWeight, int maxWeight, int connectivity)  // Non-zero seed, cap vertices at 100, weights at 10000 
		{
			...
			for (int pass = 0; pass < connectivity; pass++)
			{
				...
				List<int> unused = new ArrayList<int>();
